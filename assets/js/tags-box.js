...
var SocialPaperTagBox, array_unique_noempty;
	...
	SocialPaperTagBox = {
				...
				if ( ! disabled ) {
					...
					xbutton.on( 'click keypress', function( e ) {
						...
						if ( e.type === 'click' || e.keyCode === 13 ) {
							...
							SocialPaperTagBox.parseTags( this );
						}
					});
		...
		get : function( id ) {
			...
			$.post( ajaxurl, { 'action': 'get-tagcloud', 'tax': tax }, function( r, stat ) {
				...
				$( 'a', r ).click( function() {
					SocialPaperTagBox.flushTags( $( '#' + tax ), this );
					return false;
				});
		...
		init : function() {
			...
			$('.tagsdiv').each( function() {
				SocialPaperTagBox.quickClicks(this);
			});
			...
			$('input.newtag', ajaxtag).keyup(function(e){
				if ( 13 == e.which ) {
					SocialPaperTagBox.flushTags( $(this).closest('.tagsdiv') );
					return false;
				}
			}).keypress(function(e){
				if ( 13 == e.which ) {
			...
			$('#post').submit(function(){
				$('div.tagsdiv').each( function() {
					SocialPaperTagBox.flushTags(this, false, 1);
				});
			});

			// tag cloud
			$('.tagcloud-link').click(function(){
				SocialPaperTagBox.get( $(this).attr('id') );
